...
class FutonQuizSingleCollection {
  ...
  async sendToKlaviyo() {
    ...
    try {
      ...
      // Step 3: Enhanced profile with custom properties + list subscription (client-side)
      if (window.klaviyoConfig.siteId) {
  ...
  /**
   * Send enhanced profile data with custom properties using client-side API
   */
  async sendEnhancedProfileData(recommendations) {
    // Use Klaviyo's client-side profiles API instead of proxy
    if (!window.klaviyoConfig.siteId) {
      console.warn('Klaviyo site ID not configured for enhanced profile data');
      return;
    }
    ...
    try {
      // Use Klaviyo's client-side profiles endpoint
      const response = await fetch(`https://a.klaviyo.com/client/profiles/?company_id=${window.klaviyoConfig.siteId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        if (window.klaviyoConfig.debug) {
          console.log('Enhanced profile data sent successfully via client API');
        }
      } else {
        console.error('Failed to send enhanced profile data:', response.status, await response.text());
      }
    } catch (error) {
  ...
  /**
   * Add user to Klaviyo list using client-side subscription API
   */
  async addToKlaviyoList() {
    // Use the existing client-side subscription method
    await this.subscribeToKlaviyoList();
  }
  ...
  async subscribeToKlaviyoList() {
    ...
    if (!window.klaviyoConfig.siteId) {
      console.warn('Klaviyo site ID not configured');
      return;
    }
    ...
    try {
      if (window.klaviyoConfig.debug) {
        ...
        console.log('Using site ID:', window.klaviyoConfig.siteId);
        console.log('User email:', this.quizData.contactInfo.email);
        console.log('Marketing consent:', this.quizData.contactInfo.marketingConsent);
      }

      // Helper: fetch profile id by email if creation returns empty body
      const getProfileIdByEmail = async (email) => {
        try {
          const filter = encodeURIComponent(`equals(email,"${email}")`);
          const url = `https://a.klaviyo.com/client/profiles/?company_id=${window.klaviyoConfig.siteId}&filter=${filter}`;
          ...
          if (!resp.ok) {
            if (window.klaviyoConfig.debug) {
              console.log('Profile lookup by email failed:', resp.status, await resp.text());
            }
            return null;
          }
          ...
          if (window.klaviyoConfig.debug) {
            console.log('Profile lookup by email result id:', id);
          }
          return id;
        } catch (e) {
          console.error('Error looking up profile by email:', e);
          return null;
        }
      };
      ...
      // Fallback: lookup profile id by email if still missing
      if (!profileId) {
        profileId = await getProfileIdByEmail(this.quizData.contactInfo.email);
      }
      ...
      if (window.klaviyoConfig.debug) {
        console.log('Subscribing with payload:', JSON.stringify(subscriptionPayload, null, 2));
      }
      ...
      if (response.ok) {
        ...
        window.klaviyo.push(['track', 'List Subscription Success', {
          'List ID': window.klaviyoConfig.listId,
          'Email': this.quizData.contactInfo.email,
          'Source': 'futon-quiz',
          'Success': true,
        ...
        window.klaviyo.push(['track', 'List Subscription Failed', {
          'List ID': window.klaviyoConfig.listId,
          'Email': this.quizData.contactInfo.email,
          'Source': 'futon-quiz',
          'Success': false,
          'Error Status': response.status,
          'Error Message': errorData.substring(0, 200) // Limit error message length
        }]);
      }
      ...
      window.klaviyo.push(['identify', {
        ...
        '$last_name': this.quizData.contactInfo.name.split(' ').slice(1).join(' ') || '',
        '$phone_number': this.quizData.contactInfo.phone || undefined,
        '$consent': this.quizData.contactInfo.marketingConsent ? ['email'] : [],
        '$source': 'futon-quiz',
        '$list_subscription_attempted': true,
        '$target_list': window.klaviyoConfig.listId
      }]);
      ...
      window.klaviyo.push(['track', 'List Subscription Error', {
        'List ID': window.klaviyoConfig.listId,
        'Email': this.quizData.contactInfo.email,
        'Source': 'futon-quiz',
        'Success': false,
      ...
      // Step 2: Subscribe to list using correct JSON:API structure
      const subscriptionPayload = {
        data: {
          type: 'subscription',
          attributes: {
